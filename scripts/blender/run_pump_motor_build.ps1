# Headless close-coupled pump+motor asset build → library
$ErrorActionPreference = 'Stop'
$Root = Resolve-Path (Join-Path $PSScriptRoot '..\..')
$Blender = Join-Path $Root 'tools\blender-4.5.10\blender-4.5.10-windows-x64\blender.exe'
$Script = Join-Path $Root 'scripts\blender\build_pump_motor.py'
$OutDir = Join-Path $Root 'assets\blender\library\pumps\close-coupled-motor'
